#!/usr/bin/env python3
"""Extract original批改 content from conversation history and update reports.json"""

import json

# Original批改 content extracted from conversation history (6月13日-15日)
...
for report in data['reports']:
    report_id = report['id']
    if report_id in original_content:
        for dim in ['D1', 'D2', 'D3', 'D4', 'D5', 'D6', 'D7', 'D8']:
            if dim in original_content[report_id]:
                report['details'][dim]['highlights'] = original_content[report_id][dim]['highlights']
                report['details'][dim]['shortcomings'] = original_content[report_id][dim]['shortcomings']
                report['details'][dim]['original'] = original_content[report_id][dim]['original']

# Save updated reports
with open('data/reports.json', 'w', encoding='utf-8') as f:
    json.dump(data, f, ensure_ascii=False, indent=2)

print(f"Updated {len(data['reports'])} reports with original content")
